...
const TabBar: React.FC<BottomTabBarProps> = ({ state, descriptors, navigation }) => {
    ...
    const icons: Record<string, (props: Partial<IconProps<any>>) => JSX.Element> = {
        ...
        groups: (props) => <FontAwesome6 name="users" size={26} {...props} />,
        crisis: (props) => <Foundation name="alert" size={30} {...props} />,
    ...
    const visibleRoutes = state.routes.filter(route =>
        ['home', 'groups', 'crisis', 'agenda', 'services'].includes(route.name)
    );
    ...
    return (
        <View style={[styles.tabbar, { paddingBottom: insets.bottom + 15 }]}>
            {visibleRoutes.map((route, idx) => {
                ...
                const iconColor = route.name === 'crisis'
                    ? light.iconAlert
                    : isFocused
